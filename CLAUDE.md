...
This is background information provided by Simon.

### Origin

This repo is forked from Google's PyTorch implementation of the Gemma LLM - including vision -
for Gemma models 1, 2 and 3. This is an inference only implementation, and is minimal. It is not
based on HuggingFace transformers.

### Usage

The relevant model or models will be used in an iOS app running ob iPhone or iPad. In future
I may also support macOS, running only on Apple Silicon. Therefore, in all cases the Apple
Neural Engine will be available, and should be exploited as much as possible. This means that
the CoreML framework must be used to run models, and coremltools used to convert PyTorch models.
When required, please reference:

[Apple Neural Engine (ANE) Transformers](https://github.com/apple/ml-ane-transformers?tab=readme-ov-file#apple-neural-engine-ane-transformers)

Since much of the app's logic will be expressed through prompts, and controlled through the
LLM, the top-level model to be used will be FunctionGemma.
When required, please reference:
